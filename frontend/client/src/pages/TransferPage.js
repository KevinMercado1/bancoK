...
const TransferPage = () => {
  ...
  const { destinationAccountNumber, amount } = formData;
  ...
  const handleTransfer = async (e) => {
    ...
    try {
      const res = await axios.post(
        'http://localhost:5007/api/transactions/transfer',
        { destinationAccountNumber, amount }
      );
      setUser((prevUser) => ({ ...prevUser, balance: res.data.newBalance }));
      setMessage(
        `Transferencia exitosa. Nuevo saldo: $${parseFloat(
          res.data.newBalance
        ).toFixed(2)}`
      );
      setFormData({ destinationAccountNumber: '', amount: '' });
    } catch (err) {
      setError(
